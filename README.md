...
A full-stack tool for extracting, categorizing, and tracking transactions from bank statement PDFs across multiple accounts and months.

## Stack
React 18 (Vite) · FastAPI · PostgreSQL · SQLAlchemy (async) · Docker · JWT-based auth (fastapi-users)

## What it does
- Parses FNB (First National Bank) PDF statements into structured transaction data
...
## Current limitations
- PDF parsing is currently built for FNB's statement format specifically; extending to other banks would mean adding a parser per provider
...
- `statement_parser.py` — FNB PDF parsing logic
